...
# ------------------------------------------------------------
# 20260407 - Weibull MP Programm 2
# ------------------------------------------------------------
# Intervall: 2
# Obere Intervallgrenze: 16.629256 Mio. Zyklen
# Öffnungsdauer: 6.00 min
...
#   Programm 10 auf 180 Takte umskaliert.
#
# Befehlslogik:
#   R        = Referenzfahrt
...
steps_per_tick = [
    ...
    13, 12, 12, 13, 13, 13, 13, 14, 13, 14,
    14, 14, 15, 14, 15, 15, 15, 16, 15, 16,
    16, 16, 17, 17, 17, 17, 17, 18, 18, 18,
    19, 18, 19, 20, 19, 20, 20, 20, 21, 21,
    21, 22, 22, 22, 22, 23, 23, 23, 24, 24,
]

assert len(steps_per_tick) == 180, "Es müssen genau 180 Takte sein."
...
try:
    ...
    for i, step_count in enumerate(steps_per_tick, start=1):
        if step_count > 0:
            ...
            print(f"Takt {i:03d}/180 | Öffne um {step_count:2d} Schritte | kumulativ offen: {opened_steps}")
        else:
            print(f"Takt {i:03d}/180 | keine Bewegung | kumulativ offen: {opened_steps}")
